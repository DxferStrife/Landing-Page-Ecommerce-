...
(() => {
    ...
    const canvas = hero.querySelector(".hero-brand-grid");
    ...
    if (canvas && ctx) {
        ...
        const draw = () => {
            ...
            if (intensity < 0.01)
                return;
            const strokeSegment = (x1, y1, x2, y2) => {
        ...
        const tick = () => {
            ...
            draw();
            const settled = Math.abs(targetX - curX) < 0.5 &&
    ...
    const typed = hero.querySelector(".hero-brand-typed");
    const typedText = hero.querySelector(".hero-brand-typed-text");
