...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll<HTMLElement>(
      ".servicios-grid, .proceso-grid, .testimonios-grid, .proyectos-grid, .precios-grid, .faq-list"
    )
    .forEach((grupo) => {
      const tarjetas = Array.from(grupo.children) as HTMLElement[];
      if (!tarjetas.length) return;
      gsap.set(tarjetas, { y: 48, opacity: 0 });
      ScrollTrigger.batch(tarjetas, {
        start: "top 88%",
        once: true,
        onEnter: (elementos: Element[]) =>
          gsap.to(elementos, { y: 0, opacity: 1, duration: 0.7, ease: "power2.out", stagger: 0.12 }),
      });
    });

  // Bloques "caso" (caso de éxito, detalle de planes): el texto siempre entra desde abajo.
  document.querySelectorAll<HTMLElement>(".caso-copy").forEach((copy) => {
    gsap.from(copy, {
      y: 48,
      opacity: 0,
      duration: 0.7,
  ...
  mm.add("(max-width: 899px)", () => {
    document.querySelectorAll<HTMLElement>(".caso-figure").forEach((figura) => {
      gsap.from(figura, {
        y: 48,
        opacity: 0,
        duration: 0.7,
